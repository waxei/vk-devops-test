...
- Используется счетчик ошибок: при достижении порога `MAX_RETRIES` выполняется перезапуск (защита от флаппинга)
...
- Приложение запускается от непривилегированного пользователя `monitoring-user` для минимизации рисков
- Мониторинг требует прав root для выполнения `systemctl restart`
- Настроены правильные права доступа к файлам и директориям
...
- Python 3
- Система с systemd
- Наличие build-essentials (или development tools для yum)
...
После изменения конфигурации можно переустановить систему (это автоматически перезапустит сервисы):

```bash
sudo make install
```

Или перезапустить сервисы вручную:
